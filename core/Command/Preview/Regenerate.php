...
class Regenerate extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		$lastFileId = $this->config->getAppValue('preview', 'last_file_id', '1723343');
		$limit = 100;

		for ($i = 0; $i < 10; $i++) {
			$start = microtime(true);
			if ($lastFileId !== 'none') {
				$result = $this->connection->executeQuery('SELECT * FROM *PREFIX*filecache WHERE path NOT LIKE "appdata%" AND fileid > ? AND mimetype in (SELECT id from *PREFIX*mimetypes WHERE mimetype LIKE "image%") ORDER BY fileid ASC LIMIT ?', [(int)$lastFileId, $limit], [IQueryBuilder::PARAM_INT, IQueryBuilder::PARAM_INT]);
			} else {
				$result = $this->connection->executeQuery('SELECT * FROM *PREFIX*filecache WHERE path NOT LIKE "appdata%" AND mimetype in (SELECT id from *PREFIX*mimetypes WHERE mimetype LIKE "image%") ORDER BY fileid ASC LIMIT ?', [$limit], [IQueryBuilder::PARAM_INT]);
			}
			$end = microtime(true);
